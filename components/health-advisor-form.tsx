...
export function HealthAdvisorForm({
  surfaceClassName = "rounded-[28px] border border-[#deece9] bg-white p-5 shadow-[0_16px_36px_rgba(16,24,40,0.06)] md:p-6",
  ...
  return (
    <div className={surfaceClassName}>
      <div className="mb-5">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#0f8f7c]">Talk to a Health Advisor</p>
        <h3 className="mt-2 text-2xl font-bold text-[#102a2d] md:text-3xl">Get expert guidance in just 2 minutes</h3>
        <p className="mt-2 text-sm leading-7 text-[#5a7273]">
      ...
      <div className="mb-5 grid gap-2.5">
        {[
          ...
          <div
            key={item}
            className="flex items-center gap-2 rounded-2xl border border-[#e3efed] bg-[#f7fbfa] px-3 py-2.5 text-sm font-medium text-[#35595b]"
          >
            <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#eaf8f5] text-[#0f8f7c]">
      ...
      <form className="space-y-4" onSubmit={onSubmit}>
        <input
          ...
          className="w-full rounded-2xl border border-[#dbe9e7] bg-white px-4 py-3 text-sm text-[#102a2d] outline-none transition focus:border-[#0f8f7c]"
        />
            ...
            <button
              ...
              className={`rounded-2xl border px-3 py-3 font-semibold uppercase tracking-[0.08em] transition ${
                purpose === "before_test"
                  ...
                  : "border-[#dbe9e7] bg-white text-[#35595b]"
              }`}
            >
              Before Test
            </button>
      ...
      {message ? (
        <p className={`mt-3 text-sm ${status === "success" ? "text-[#0f8f7c]" : "text-red-600"}`}>{message}</p>
      ) : null}
